...
TitleState::TitleState(){
	...
	SpriteRenderer* newspr = new SpriteRenderer((*bg), "Recursos/img/Title.png", 1, 1);
	...
	Text* newtxt=new Text((*text),"Recursos/font/neodgm.ttf", 100, Text::TextStyle::SOLID, "Press Space To Start", {255,255,255,255}, 1);
	text->AddComponent(newtxt);
	//CameraFollower* newflwr2=new CameraFollower(*text);
	//text->AddComponent(newflwr2);
	text->box.y=Camera::pos.y+750;
	text->box.x=Camera::pos.x+100;
	AddObject(text);
	return;
}
