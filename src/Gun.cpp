	...
	if (cooldown == 1) {
		if (cdtimer.Get() > 0.3) {
			...
			if (associated.angleDeg > 90 || associated.angleDeg < -90) {
				((Animator*)associated.GetComponent("Animator"))->SetAnimation("reloadingFlip");
			}
			else {
				((Animator*)associated.GetComponent("Animator"))->SetAnimation("reloading");
			}
			reloadSound.Play(1);
			cooldown++;
		}
		...
		if (cdtimer.Get() > 0.5) {
			cdtimer.Restart();
			if (associated.angleDeg > 90 || associated.angleDeg < -90) {
				((Animator*)associated.GetComponent("Animator"))->SetAnimation("idleFlip");
			}
			else {
